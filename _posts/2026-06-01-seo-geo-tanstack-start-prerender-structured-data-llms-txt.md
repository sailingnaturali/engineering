...
> **Draft — not yet published.** Publishing is gated on the apex (`sailingnaturali.com`) being indexed in Google Search Console first, per our launch-order rule. Until the new site is in the index, holding this post avoids pointing the dev audience at a domain mid-cutover.
...
{
  "scripts": {
    "prebuild": "tsx scripts/generate-seo-assets.mjs",
    "build": "vite build"
  }
}
```
...
> All-electric eco-charter in the Pacific Northwest, built in the open.

## Core pages
- [The boat](https://sailingnaturali.com/boat): the all-electric catamaran and its systems
- [The voyage](https://sailingnaturali.com/voyage): the delivery route and the plan
```

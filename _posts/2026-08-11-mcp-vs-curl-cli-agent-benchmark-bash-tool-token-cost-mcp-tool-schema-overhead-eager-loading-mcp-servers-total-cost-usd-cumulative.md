...
- **Small models may still want shaped tools.** This is Sonnet, n=12, one vessel. A separate n=10 probe against a local `qwen3.6` suggests the picture changes below the frontier: raw value retrieval was correct in every run — it can fetch a number — but *interpreting* one, "is the battery charging or discharging?", scored **3/10 with thinking off** (one run degenerated into `...`) and **10/10 with thinking on low**. Retrieval is not the hard part; the semantics on top of it are, and that is exactly where a tool returning `"discharging"` instead of `-3.1` earns its keep. Re-measure before betting an on-vessel local model on these results.
